...
class InvertedIndexTitle:  
    ...
    def posting_lists_iter(self):
        ...
        with closing(MultiFileReader()) as reader:
            for w, locs in self.posting_locs.items():
                b = reader.read(locs, self.df[w] * TUPLE_SIZE, 'postings_gcp_title')
                posting_list = []
    ...
    def write_a_posting_list(b_w_pl, bucket_name):
        ...
        with closing(MultiFileWriter(".", bucket_id, bucket_name)) as writer:
            ...
            InvertedIndexTitle._upload_posting_locs(bucket_id, posting_locs, bucket_name)
        return bucket_id

    
    @staticmethod
    def _upload_posting_locs(bucket_id, posting_locs, bucket_name):
        with open(f"{bucket_id}_posting_locs_title.pickle", "wb") as f:
            pickle.dump(posting_locs, f)
        client = storage.Client()
        bucket = client.bucket(bucket_name)
        blob_posting_locs = bucket.blob(f"postings_gcp_title/{bucket_id}_posting_locs_title.pickle")
        blob_posting_locs.upload_from_filename(f"{bucket_id}_posting_locs_title.pickle")
